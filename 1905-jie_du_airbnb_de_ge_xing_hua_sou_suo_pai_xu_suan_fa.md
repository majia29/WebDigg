...
a\) 区别对待是否以成交行为作为序列结束的点击序列。如果点击序列不是以成交行为作为序列结束的点击序列，则优化函数沿用式（3），否则，优化函数调整为如下：
...
因此，每一条成交序列里的元素，由原来的房源变成了房源聚类结果。为了方便起见，后面统称为listing\_type序列。
...
![image](images/1905-jdairbnbdgxhsspxsf-9.jpeg)
